...
/// [experimental] Build an OCI image from the current nise.toml
///
...
pub struct Build {
    ...
    #[clap(long, short, default_value = "./nise-oci", value_hint = ValueHint::DirPath)]
    output: PathBuf,
    ...
    ///
    /// By default `nise oci build` only packages tools declared in the
    ...
    /// Do not embed the currently-running nise binary at /usr/local/bin/nise
...
impl Build {
    pub async fn run(self) -> Result<()> {
        Settings::get().ensure_experimental("nise oci build")?;
...
static AFTER_LONG_HELP: &str = color_print::cstr!(
    ...
    $ <bold>nise oci build</bold>

    Build with a specific base image and tag:
    $ <bold>nise oci build --from ubuntu:24.04 --tag myorg/dev:latest -o ./img</bold>

    Inspect the result with skopeo:
    $ <bold>skopeo inspect oci:./nise-oci</bold>

    Push to a registry:
    $ <bold>skopeo copy oci:./nise-oci docker://ghcr.io/me/dev:latest</bold>
...
<bold><underline>Notes:</underline></bold>
    ...
    - The host nise binary is embedded at /usr/local/bin/nise by default;
      build on the same OS/arch as your target image (or pass --no-nise).
"#
);
