...
/// If `$MISE_GLOBAL_CONFIG_FILE` is set to anything that ends in `.toml`, it will be parsed as `nise.toml`.
/// Otherwise, it will be parsed as a `.tool-versions` file.
///
/// Use MISE_ASDF_COMPAT=1 to default the global config to ~/.tool-versions
///
/// Use `nise local` to set a tool version locally in the current directory.
...
pub struct Global {
    ...
    /// e.g.: `nise global --fuzzy node@20` will save `node 20` to ~/.tool-versions
    ...
    /// e.g.: `nise global --pin node@20` will save `node 20.0.0` to ~/.tool-versions
...
static AFTER_LONG_HELP: &str = color_print::cstr!(
    ...
    $ <bold>nise global --fuzzy node@20</bold>

    # set the current version of node to 20.x
    # will use a precise version (e.g.: 20.0.0) in .tool-versions file
    $ <bold>nise global --pin node@20</bold>

    # show the current version of node in ~/.tool-versions
    $ <bold>nise global node</bold>
    20.0.0
"#
);
