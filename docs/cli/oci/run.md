...
By default, both the container (`--rm`) and the loaded image are removed when the command exits, so repeated `nise oci run` calls don't accumulate images in podman / docker storage. Pass `--keep` to retain the image under the tag nise used (`nise-oci:run-*` for docker; the pulled image ID for podman).
...
Note: unlike `docker run -v`, there's no `-v` short flag here because nise reserves `-v` for --verbose. Use `--volume` or `--mount`.
...
$ nise oci run -it -- bash

Run a one-shot command with env + volume (note: `-v` is reserved
for --verbose, so use `--volume`):
$ nise oci run -e DEBUG=1 --volume $PWD:/work -w /work -- npm test

Re-use a previously built layout (skip the build step):
$ nise oci build -o ./img && nise oci run --image-dir ./img -- node -e 'console.log(process.version)'
```
