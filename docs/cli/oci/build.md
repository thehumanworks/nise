...
$ nise oci build --from ubuntu:24.04 --tag myorg/dev:latest -o ./img

Inspect the result with skopeo:
$ skopeo inspect oci:./nise-oci

Push to a registry:
$ skopeo copy oci:./nise-oci docker://ghcr.io/me/dev:latest
```

Notes:

```
- The image only contains tools from the project's nise config (and
