<!-- @generated by usage-cli from usage spec -->
# `nise bootstrap packages brew tap`

- **Usage**: `nise bootstrap packages brew tap [FLAGS] <TAP> [URL]`
- **Source code**: [`src/cli/bootstrap/packages/brew/tap.rs`](https://github.com/jdx/mise/blob/main/src/cli/bootstrap/packages/brew/tap.rs)

Add a Homebrew tap URL to [bootstrap.brew.taps]

## Arguments

### `<TAP>`

Tap name, e.g. `owner/repo`

### `[URL]`

GitHub URL for the tap. Defaults to https://github.com/&lt;owner>/homebrew-&lt;repo>.git

## Flags

### `-l --local`

Write to the local config instead of the global config

### `-n --dry-run`

Print the config change without writing it

### `-p --path <PATH>`

Write to this config file or directory

Examples:

```
$ nise bootstrap packages brew tap railwaycat/emacsmacport
$ nise bootstrap packages brew tap acme/tools https://github.com/acme/homebrew-tools.git
```
