...
# `nise bootstrap packages status`

- **Usage**: `nise bootstrap packages status [-J --json] [--missing]`
- **Aliases**: `ls`
...
```
$ nise bootstrap packages status
$ nise bootstrap packages status --json
$ nise bootstrap packages status --missing # exit 1 if anything is out of sync
```
