...
- **Usage**: `nise exec [FLAGS] [TOOL@VERSION]… [-- COMMAND]…`
- **Aliases**: `x`
- **Source code**: [`src/cli/exec.rs`](https://github.com/jdx/mise/blob/main/src/cli/exec.rs)

Execute a command with tool(s) set

use this to avoid modifying the shell session or running ad-hoc commands with nise tools set.
...
```
$ nise exec node@20 -- node ./app.js  # launch app.js using node-20.x
$ nise x node@20 -- node ./app.js     # shorter alias

# Specify command as a string:
$ nise exec node@20 python@3.11 --command "node -v && python -V"

# Run a command in a different directory:
$ nise x -C /path/to/project node@20 -- node ./app.js
```
