...
# `nise generate tool-stub`

- **Usage**: `nise generate tool-stub [FLAGS] <OUTPUT>`
...
Wrap stub in a bootstrap script that installs nise if not already present

When enabled, generates a bash script that:
1. Checks if nise is installed at the expected path
2. If not, downloads and installs nise using the embedded installer
3. Executes the tool stub using nise

### `--bootstrap-version <BOOTSTRAP_VERSION>`

Specify nise version for the bootstrap script
...
$ nise generate tool-stub ./bin/gh --url "https://github.com/cli/cli/releases/download/v2.336.0/gh_2.336.0_linux_amd64.tar.gz"

Generate a tool stub with platform-specific URLs:
$ nise generate tool-stub ./bin/rg \
...
$ nise generate tool-stub ./bin/node \
...
$ nise generate tool-stub ./bin/tool \
...
$ nise generate tool-stub ./bin/tool --url "https://example.com/tool.tar.gz" --skip-download

Fetch checksums for an existing stub:
$ nise generate tool-stub ./bin/jq --fetch
# This will read the existing stub and download files to fill in any missing checksums/sizes

Generate a bootstrap stub that installs nise if needed:
$ nise generate tool-stub ./bin/tool --url "https://example.com/tool.tar.gz" --bootstrap
# The stub will check for nise and install it automatically before running the tool

Generate a bootstrap stub with a pinned nise version:
$ nise generate tool-stub ./bin/tool --url "https://example.com/tool.tar.gz" --bootstrap --bootstrap-version 2025.1.0

Lock an existing tool stub with pinned version and platform URLs/checksums:
$ nise generate tool-stub ./bin/node --lock

Bump the version in a locked stub:
$ nise generate tool-stub ./bin/node --lock --version 22
