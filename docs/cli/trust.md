...
- **Usage**: `nise trust [FLAGS] [CONFIG_FILE]`
- **Source code**: [`src/cli/trust.rs`](https://github.com/jdx/mise/blob/main/src/cli/trust.rs)

Marks a config file as trusted

This means nise is allowed to parse the file when it needs to read config
that may execute code or affect the environment. nise checks trust before
parsing `nise.toml`. Without trust, nise may prompt, skip the config in some
