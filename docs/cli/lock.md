...
Check nise.lock instead of updating nise.lock
...
Update nise.local.lock instead of nise.lock
...
```
$ nise lock                       # update lockfile for all common platforms
$ nise lock node python           # update only node and python
$ nise lock --platform linux-x64  # update only linux-x64 platform
$ nise lock --dry-run             # show what would be updated
$ nise lock --minimum-release-age 2024-01-01   # lock latest/fuzzy versions released before 2024-01-01
$ nise lock --local               # update nise.local.lock for local configs
$ nise lock --global              # update only global config lockfiles
```
